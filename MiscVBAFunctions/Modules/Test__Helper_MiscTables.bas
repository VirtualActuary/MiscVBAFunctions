...
Function TestHasLO_and_GetLO(WB As Workbook)
    ...
    Pass = True = HasLO("Table1", WB) = Pass ' Correct case
    Pass = True = HasLO("taBLe1", WB) = Pass ' Should work without correct case

    Pass = "Table1" = GetLO("taBLe1", WB).Name = Pass ' Should get the correct name even with a different casing
...
Function TestTableRange(WB As Workbook)
    ...
    Pass = "Column1" = TableR.Cells(1).Value = Pass
...
Function Test_CopyTable(WB As Workbook, WB2 As Workbook)
    ...
    Pass = HasLO("TableForCopy", WB2) = Pass
    
    Pass = "General" = LOEntries(1).NumberFormat = Pass
    Pass = "0.00%" = LOEntries(2).NumberFormat = Pass
    Pass = "$#,##0.00" = LOEntries(3).NumberFormat = Pass
    Pass = "General" = LOEntries(4).NumberFormat = Pass
    Pass = "General" = LOEntries(5).NumberFormat = Pass
    Pass = "m/d/yyyy" = LOEntries(6).NumberFormat = Pass

    Pass = "=[foo]" = LOEntries(1, 1).Value = Pass
    Pass = 12 = CInt(LOEntries(1, 2).Value) = Pass
    Pass = 21 = CInt(LOEntries(2, 1).Value) = Pass
    Pass = 22 = CInt(LOEntries(2, 2).Value) = Pass
    Pass = "Hello" = LOEntries(3, 1).Value = Pass
    Pass = 32 = CInt(LOEntries(3, 2).Value) = Pass
    Pass = CVErr(xlErrName) = LOEntries(4, 1).Value = Pass
    Pass = CVErr(xlErrNA) = LOEntries(4, 2).Value = Pass
    Pass = "=foo" = LOEntries(5, 1).Value = Pass
